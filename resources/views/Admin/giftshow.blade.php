											...
											<table id="sample-table-1" class="table table-striped table-bordered table-hover">
												...
												@foreach($arr as $k => $v)
													<tr>
														...
														<td>{{$v['g_name']}}</td>
														<td class="hidden-480">{{$v['g_point']}}</td>
														<td><img src="{{url($v['g_img'])}}" width="86" alt="礼品图片" /></td>
														...
														<td>
															<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
																...
																<button class="btn btn-xs btn-danger" onclick="gift_del({{$v['g_id']}})">
...
<![endif]-->
		...
		<script>
			...
			function gift_del (id) {
				$.get("{{url('GiftListDel')}}",{'g_id':id},function (msg) {
					$("#html").html(msg);
				});
			}
		</script>
